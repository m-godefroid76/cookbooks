action :mount do
  ...
  if @new_resource.mount_folder != nil
    ...
    bash "mount s3 bucket on folder" do
      code <<-EOH
      sudo bash -c 'export AWSACCESSKEYID=#{ node[:access_key] }; export AWSSECRETACCESSKEY=#{ node[:secret_key] }; s3fs #{bucket} /var/www/html/uploads/#{folder} -o allow_other'
      EOH
    
## a instrucao not_if parece nao funcionar devidamente  quando chamada varias vezes seguidas num loop      
      not_if { ::File.directory?("/var/www/html/uploads/#{folder}/mysql/") }
    end
  end
